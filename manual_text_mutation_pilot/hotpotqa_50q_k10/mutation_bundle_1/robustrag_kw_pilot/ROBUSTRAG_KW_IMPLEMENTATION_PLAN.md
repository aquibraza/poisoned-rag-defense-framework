...
clean-passage non-abstention was **0.857** and gold-match **0.571**
...
- Self-query poison was *less* persuasive after mutation under isolation
  (strict-ASR hit rate 1.00 -> 0.60 on Ferocactus). Reducing lexical overlap is
...
### 6.2.2 Generator nondeterminism across independent generation sets

The pilot was generated twice, in two independent sets of isolated calls against
`gpt-3.5-turbo` at the repo's standard decoding settings. Comparing the two:

- **All six aggregated case outcomes on `mutated` contexts were identical**, so
  the headline strict-ASR comparison in §6.2.1 reproduced exactly.
- Clean-passage non-abstention (0.857) and gold-match (0.571) were identical.
- Cross-query dilution (0 strict-ASR hits, 2 gold matches) was identical.
- **Two things moved.** Ferocactus mutated self-query poison went 2/5 -> 3/5
  strict-ASR hits at the isolated-passage level (hence 0.40 -> 0.60 above), and
  `5ae224da554299234fd043ee` on the `original` context flipped from an answer to
  an abstention.

The lesson for scale-up: aggregated verdicts on these cases are more stable than
the per-passage vote counts underneath them, and any per-passage rate quoted from
n=5 should be treated as sampling noise until it is re-measured at scale. Nothing
here changes a §6.2.1 conclusion, but claim 3 should be stated directionally
("less persuasive under isolation") rather than with a specific ratio.
